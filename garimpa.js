...
function garimpa() {
    ...
    adicionarListaGarimpa()
}

function adicionarListaGarimpa() {
    // Esta função verifica se algum dos versículos passados pelo usuário tem hífem e tb se solicita o capítulo inteiro.
    // A listaGarimpada são todos os versículos que o usuário solicitou...
    // E desta lista, se tratará com cada passagem:
    listaGarimpada.map((cadaPassagem, indexCadaPassagem) => {
        // Se a passagem não tiver : é porque o usuário quer o capítulo inteiro (todos os versículos deste cap.)
        if (!cadaPassagem.match(":")) {
            // Caso só tenha cap. mas não versículo...
            capSemVs = `${cadaPassagem}:`
            // obtem o último versículo de um capítulo de um livro da Bíblia:
            obtemUltimoVs()
            cadaPassagem = `${capSemVs}1-${ultimoVs}`
        }

        // se no item há um hífen cap: vers a-b (devem ser mostrados os (b-a) versiculos seguintes (a mais) de b)
        // A cada passagem da lista, verifica se a passagem tenha um intervalo de versículos, i.é: Sl 23:1-4 ...
        if (cadaPassagem.match("-")) {
            // se sim:
            // execução DEFERENTE: faz uma lista com as passagens p.ex: Para Gn 1.1-2 => Gn 1.1 e Gn 1.2

            // Se por ex: Sl 23:1-4...
            // let parte livro e cap. (Sl 23:)
            let parteLivroCap = cadaPassagem.split(":")[0]
            // pega a parte: 1-4
            let parteVersiculos = cadaPassagem.split(":")[1]
            ...
            // AQUI acrecentaria mais X items na listaGarimpada a partir da posição (indexCadaPassagem) atual.
            // a = [0,1,3]
            // a.splice(2,0,2)
            // resultado: [0,1,2,3]

            let subListaPassagens = []

            for (i = versInicial; i <= versFinal; i++) {
                subListaPassagens.push(`${parteLivroCap}:${i}`)
            }

            // apaga o item da listaGarimpada que contem hifem (indicando que contém vários vers.):
            listaGarimpada.splice(indexCadaPassagem, 1)

            // Adiciona a lista de subitems dentro da listaGarimpada:
            subListaPassagens.map((a, b) =>
                listaGarimpada.splice(indexCadaPassagem + b, 0, a)
            )
        }
    })
...
function mostraVs() {
    ...
    console.log("debug - " + botaoPressionado)
    console.log(listaGarimpada)
    ...
    // CORREÇÃO     // antigo:
    // As duas variáveis abaixo são para evitar a repetição de passagens da bibliaHB, ver mais abaixo...
    // let cadaPassagemHb
    // let listaPassagensHb = []
    
    listaGarimpada.map(cadaPass=> {
        // percorre toda a biblia selecionada = todos os versículos do arq.js:
        bibliaSelecionada.map((a, b) => {
            // encontrando o versículo pedido pelo usuário dentro de acf.js ou arc.js...
            if (a[0] === cadaPass) {
                textoFinal += `<strong>${bibliaSelecionada[b][0]}:</strong> "${bibliaSelecionada[b][1]}"<br><br>`

                /*          // ANTIGO: 
                // Aqui há 2 casos: Se a bíblia é a HB então o procedimento é diferente:
                if (botaoPressionado !== "HB") {
                    ...
                    // Trecho novo:
                    // appenda numa lista as pasagens da biblia em HB (isso para evitar duplicidade)
                    // cadaPassagemHb = `<strong>${bibliaSelecionada[b][0]}:</strong> "${bibliaSelecionada[b][1]}"<br><br>`
                    // listaPassagensHb.push(cadaPassagemHb)
                    // console.log(listaPassagensHb)
                    // listaPassagensHb = Array.from(new Set(listaPassagensHb))

                    // Converte a array acima para string 
                    // poderia ser listaPassagensHb.toString() - porém ele converte com as virgulas entre os elementos do array...
                    // portanto, se usa join('') para juntar os elementos do array SEM vírgula ou qquer delimitador:
                    // textoFinal = listaPassagensHb.join('')

                    textoFinal += `<strong>${bibliaSelecionada[b][0]}:</strong> "${bibliaSelecionada[b][1]}"<br><br>`

                    // Trecho abaixo é antigo quando se tinha erro de duplicidade na impressao das passagens encontradas:
                    // aqui abaixo tem que haver uma mudança para o livro bibliaHB não repetir os versículos qdo é cap:1-n
                    // console.log(bibliaSelecionada[b+rep][1])
    ...
    // Agora manipula o HTML para receber os dados processados acima.
    ...
    if (e.altKey && (e.keyCode == 49 || e.keyCode == 97)) {
        ...
        if (selecao != '') {
            ...
            if (strongGR != undefined || strongHB != undefined) {
                ...
                if (selecaoGR.length > 0) {
                    listaMorph = selecaoGR;
                    console.log(' É morfológico... ' + listaMorph);
                    abreMorfo()
                } // / chama funcao... abre morfo no popup
                if (selecaoHB.length > 0) {
                    listaMorph = selecaoHB;
                    console.log(' É morfológico... ' + listaMorph);
                    abreMorfo()
                } // / chama funcao... abre morfo no popup 

                // SE NÃO É MORFOLÓGICO NEM STRONG ....(aqui tb pode encaixar os dicionarios thompson e nave...)
                if (selecaoGR == 0 && selecaoHB == 0) {
                    console.log('Não é morfolog. nem strong')
                } // chama funcao... para abrir passagem no popup
...
function abreStrong() {
    if (strongGR) {
        selecaoStrong = strongGR
    }
    if (strongHB) {
        selecaoStrong = strongHB
    }
